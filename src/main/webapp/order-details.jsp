...
<body>
	<div class="card">
		...
		<p>
			...
			<br> Payment: <strong>${order.paymentStatus}</strong><br>
			Method: <strong>${order.paymentMethod}</strong>
			<br> Date: <strong>${order.createdAt}</strong>
		</p>
		...
		<table>
			...
			<c:forEach var="item" items="${items}">
				<tr>
					...
					<td>${item.quantity}</td>
					<td>
						<fmt:formatNumber value="${item.finalPrice * item.quantity}" groupingUsed="true" maxFractionDigits="0" />
						VND
					</td>
				</tr>
			</c:forEach>
		...
		<div class="total-box">
			Subtotal:
			<fmt:formatNumber value="${order.subtotal}" groupingUsed="true" maxFractionDigits="0" /> VND<br>
			Shipping:
			<fmt:formatNumber value="${order.shippingFee}" groupingUsed="true" maxFractionDigits="0" /> VND<br>
			Total:
